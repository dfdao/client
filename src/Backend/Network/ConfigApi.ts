...
import { ExactArray10, ExactArray5, ExactArray8, Tuple6 } from '@darkforest_eth/settings/dist/decoder-helpers';
import {
  ArenaLeaderboard,
...
`;
  ...
  const ret = await convertData(rawData.data.arenas[0]);
...
async function convertData(config: rawConfig): Promise<LobbyInitializers> {
  return {
    ...config.config,
    START_PAUSED: true,
