...
class Repeater {
  ...
  addAttack() {
    ...
    attack.pcRemain =
      attack.pcTrigger <= attack.pcRemain ? parseInt(attack.pcTrigger / 2) : attack.pcRemain;
...
function Attack({ attack, onToggleActive, onToggleSilver, onDelete }) {
  ...
  const finalSrc =
    srcString.length > MAX_CHARS ? srcString.slice(0, MAX_CHARS - 3).concat('...') : srcString;
  const finalTarget =
  ...
  return html`
    <div style=${ActionEntry}>
      <button style=${{ border: 'none', margin: 'none' }} onClick=${onToggleActive}>
        ${attack.active ? '▶️' : '⏸️'}
      </button>
      <span>
        <span style=${{ ...Margin_3L_3R }}>
          <span style=${{ ...Clickable }} onClick=${() => centerPlanet(attack.sourceId)}
            >${finalSrc}</span
          >
          <span style=${{ ...Margin_3L_3R }}>-></span>
          <span style=${{ ...Clickable }} onClick=${() => centerPlanet(attack.targetId)}
            >${finalTarget}</span
          >
        </span>
        <span style=${{ ...Margin_3L_3R }}>
...
function AddAttack({ repeater, startFiring, toggleOutgoingFiring, toggleIncomingFiring }) {
  ...
  const toggleSendSilver = () =>
    setCurrentAttack('sendSilverStatus', toggleSilverStatus(getCurrentAttack('sendSilverStatus')));
    ...
    <div style=${{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style=${{ marginBottom: 3 }}>
        ...
        <input
          ...
          onChange=${(e) => setCurrentAttack('pcTrigger', parseInt(e.target.value))}
        />
      ...
      <div style=${{ marginBottom: 3 }}>
        ...
        <input
          ...
          onChange=${(e) => setCurrentAttack('pcRemain', parseInt(e.target.value))}
        />
        ${getCurrentAttack('pcRemain')}%
      </div>
      <div style=${{ marginBottom: 10 }}>
        Choose when to send silver:
        <button style=${{ width: 150, height: 23, fontSize: '90%' }} onClick=${toggleSendSilver}>
        ...
        <div>
          ${repeater.outgoingStatus() === FIRING_NONE
            ...
            : html`
                ...
                >
                  ${repeater.outgoingStatus() === FIRING_PAUSED ? 'Resume' : 'Pause'} Firing
                  <span style=${Keyboard_Shortcut}>[${KEY_TOGGLE_OUTGOING_FIRING_DISPLAY}]</span>
                </button>
              `}
          ${repeater.incomingStatus() === FIRING_NONE
            ...
            : html`
                <button style=${{ ...Margin_12B, width: 210 }} onClick=${toggleIncomingFiring}>
                  ${repeater.incomingStatus() === FIRING_PAUSED ? 'Resume' : 'Pause'} Being Fired At
                  <span style=${Keyboard_Shortcut}>[${KEY_TOGGLE_INCOMING_FIRING_DISPLAY}]</span>
                </button>
              `}
        </div>
      </div>
...
const drawHighlights = (plugin) => {
  ...
  const drawHighlight = (planetId, rgba, periodMs, lineWidth, arcFraction) => {
    ...
    const cR =
      10 + viewport.worldToCanvasDist(1.4 * ui.getRadiusOfPlanetLevel(thePlanet.planetLevel));
