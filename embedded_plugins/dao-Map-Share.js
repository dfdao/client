...
class RemotePlugin extends Plugin {
    constructor() {
        ...
        this.baseUrl = 'http://3.211.163.213:8500';
    }
    ...
    onUploadToServer = async () => {
        ...
        try {
            ...
            this.status.innerText = 'Map exported & shared!';
            this.status.style.color = 'white'
        } catch (err) {
            console.error(err);
            this.status.innerText = 'Failed to share the map';
            this.status.style.color = 'red';
        }
    }
    ...
    onDownloadToServer = async () => {
        ...
        this.status.innerText = 'Fetched map data! Importing into game, this will take awhile...';
        this.status.style.color = 'white';
