...
import { PlanetLevel, PlanetType, SpaceType } from 'https://cdn.skypack.dev/@dfdao/types';

import { html, render, useState } from 'https://unpkg.com/htm/preact/standalone.module.js';

import { getPlayerColor } from 'https://cdn.skypack.dev/@dfdao/procedural';
...
function mapFilterExport() {
  ...
  function generateMapWithoutBackground() {
    ...
    for (let i = 0; i < chunksAsArray.length; i++) {
      ...
      for (let i = 0; i < planetLocationsWithMark.length; i++) {
        ...
        if (
          i === planetLocationsWithMark.length - 1 ||
          ...
          for (let j = 0; j < stack.length; j++) {
            ...
            if (
              pltCoords.x >= tx &&
              pltCoords.x < tx + smallLength &&
              pltCoords.y >= ty &&
              pltCoords.y < ty + smallLength
            );
            else notCnt++;
          }
  ...
  let onDownloadWithoutBackground = async () => {
    let newInfo = html`<div style=${{ color: '#CCFFFF' }}>
      Begin To Download Map Without Background
    </div>`;
    setInfo(newInfo);
    let newInfo2 = html`<div style=${{ color: '#CCFFFF' }}>
  ...
  async function processMap(input) {
    ...
    try {
      ...
      console.error(err);
      let newInfo = html`<div style=${{ color: 'red' }}>
  ...
  let onUpload = async () => {
    ...
    inputFile.onchange = () => {
      try {
        ...
        newInfo2 = html`<div style=${{ color: '#FFFF00' }}>
  ...
  const rightLevelSelect = html`
    <select
      style=${selectStyle}
      value=${rightLevel}
      onChange=${(e) => setRightLevel(e.target.value)}
    >
      ${PLANET_LEVELS.map(({ value, text }) => html`<option value=${value}>${text}</option>`)}
    </select>
  `;
  ...
  let theAsteroidFiledComponent = html`<div>
    <input
      type="checkbox"
      checked=${hasAsteroidField}
      onChange=${() => setHasAsteroidField(!hasAsteroidField)}
    />
    ${' AsteroidField'}
  </div>`;
  ...
  let theSpacetimeRipComponent = html`<div>
    <input
      type="checkbox"
      checked=${hasSpacetimeRip}
      onChange=${() => setHasSpacetimeRip(!hasSpacetimeRip)}
    />
    ${' SpacetimeRip'}
  </div>`;
  ...
  let planetTypeComponent = html`
    <div style=${{ marginLeft: '20px', textAlign: 'left', float: 'left' }}>
      ${thePlanetComponent} ${theAsteroidFiledComponent} ${theFoundryComponent}
      ${theSpacetimeRipComponent} ${theQuasarComponent}
    </div>
  `;
  ...
  let theBlackSpaceComponent = html`<div>
    <input
      type="checkbox"
      checked=${ohBlackSpace}
      onChange=${() => setOhBlackSpace(!ohBlackSpace)}
    />
    ${' Black'}
  </div>`;
  ...
  let theGreenSpaceComponent = html`<div>
    <input
      type="checkbox"
      checked=${ohGreenSpace}
      onChange=${() => setOhGreenSpace(!ohGreenSpace)}
    />
    ${' Green'}
  </div>`;
  ...
  let theDarkblueSpaceComponent = html`<div>
    <input
      type="checkbox"
      checked=${ohDarkblueSpace}
      onChange=${() => setOhDarkblueSpace(!ohDarkblueSpace)}
    />
    ${' Darkblue'}
  </div>`;

  let spaceTypeComponent = html` <div
    style=${{ marginLeft: '20px', textAlign: 'left', float: 'left' }}
  >
    ${theBlackSpaceComponent} ${theGreenSpaceComponent} ${theBlueSpaceComponent}
    ${theDarkblueSpaceComponent}
  </div>`;

  let ownerComponent = html` <div style=${{ marginLeft: '20px', textAlign: 'left', float: 'left' }}>
  ...
  return html`<div style=${divStyle}>
    ...
    <div style=${{ marginTop: '5px' }}>
      <button style=${longButtonStyle} onClick=${onDownloadWithoutBackground}>
        Download Map Without Background
      </button>
    </div>
    <div style=${{ marginTop: '5px' }}>
      <button style=${longButtonStyle} onClick=${onDownloadWithBackground}>
        Download Map With Background
      </button>
    </div>
    <div style=${{ marginTop: '5px' }}>
