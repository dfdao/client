...
const getRgb = (arr, hover, invert = false) => {
  const [r, g, b] = arr.map((c) =>
    hover ? (invert ? 255 - c : c) : Math.max(0, (invert ? 255 - c : c) - NON_HOVER_DIMMER)
  );
  return `rgb(${r}, ${g}, ${b})`;
};
...
const planetHasRelevantSpaceship = (plugin, planet) =>
  planet.heldArtifactIds.some((id) => plugin.spaceships.some((spaceship) => id === spaceship.id));
...
const mainChecks = (plugin, planet) =>
  levelInRange(plugin, planet) && distanceInRange(plugin, planet) && !planet.destroyed;
  ...
  if (isToggledOn(planetArr)) {
    for (let planet of planetArr) {
      ...
      if (!drawOptions.line.value) {
        ctx.fillStyle = `rgba(${'' + colRgbArr[0]}, ${'' + colRgbArr[1]}, ${'' + colRgbArr[2]}, ${
          '' + colAlpha
        })`;
      } else {
        ctx.strokeStyle = `rgba(${'' + colRgbArr[0]}, ${'' + colRgbArr[1]}, ${'' + colRgbArr[2]}, ${
          '' + colAlpha
        })`;
        radius *= EXTRA_RADIUS_WORLD_PX_LINE_MULTIPLY;
        ctx.lineWidth =
          (BASE_LINE_WIDTH_PX + LINE_WIDTH_PLANET_LEVEL_FACTOR * planet.planetLevel) *
      ...
      const cR =
        extraRadiusFactor * (viewport.worldToCanvasDist(radius) + EXTRA_RADIUS_CANVAS_PX_ADD);
